...
import com.example.testnavigation.adapter.mine.Commentadapter2;
 ...
 */
public class CommentFragment extends BaseFragment<ListNotify.ListNotifyV, IPUserListNofity<ListNotify.ListNotifyV>> implements ListNotify.ListNotifyV {
    ...
    public void showListNotifyTab(ListNotifyBean listNotifyBean) {
        ...
        for (int i = 0; i < listNotifyBean.getData().size(); i++) {
            if (listNotifyBean.getData().get(i).getNotifyType().equals("2")) {
        ...
        Commentadapter2 commentadapter2 = new Commentadapter2(mData);
        mRlv.setAdapter(commentadapter2);
    }
